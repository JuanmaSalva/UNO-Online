...
public:
     ...
     */
    int receive(Serializable &obj, Socket * &sock);

    int receive(Serializable &obj) //Descarta los datos del otro extremo
    {
        Socket * s = 0;

        return receive(obj, s);
    }
     ...
     */
    int sen(Serializable& obj, const Socket& sock);
